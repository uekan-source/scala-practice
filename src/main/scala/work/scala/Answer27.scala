...
object Answer27:
  ...
  def transferMoney(
    ...
    for {
      source <- EitherT(accountFind(idSource, toAccountsMap))
      money <- EitherT.fromEither[Future](confirmAccountBalance(source, amount))
      target <- EitherT(accountFind(idTarget, toAccountsMap))
    } yield (money, target.balance + amount)
