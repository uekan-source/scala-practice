...
object Answer28:
  ...
  def searchAll(
    ...
    for {
      odr <- EitherT(searchOrder(orderId, orders))
      csm <- EitherT(searchCustomer(odr.customerId, customers))
      pdt <- EitherT(searchProduct(odr.productId, products))
    } yield OrderProduct(
        csm.name,
        pdt.name,
        odr.quantity,
        odr.quantity * pdt.price
      )
